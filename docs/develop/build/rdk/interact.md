---
title: "Interact with RDK RollApp"
slug: interact
---
...
We will add a `--broadcast-mode block` to each transaction so that we'll get the transaction result and not just an approval that it was received.
...
Wait 5 minutes for game expiration (you can also actually win the game but it's difficult without UI-client):

...
<Tabs>
<TabItem value="Devnet Hub" label="Devnet Hub">
...
```
dymd start --home ~/.roller/local-hub
```

While the Dymension Hub full node is running, publish the following transaction via CLI:

```
dymd tx bank send local-user <ADDRESS-TO-FUND> 1udym --yes -b block --keyring-backend test --home ~/.roller/local-hub
```

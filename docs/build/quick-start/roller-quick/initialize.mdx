...
In Dymension's [discord](https://discord.gg/dymension) please fund the Dymension Hub addresses in the `faucet` channel with the following command as an example:
...
Additionally, for Celestia network tokens users may use the `celestia-faucet` on the Dymension discord with the following command as an example:
